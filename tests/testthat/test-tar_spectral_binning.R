test_that("tar_input_spectral_binning() works", {
    targets <- tar_spectral_binning(test)
    
    expect_identical(class(targets),'list')
})

test_that("tar_input_spectral_binning() throws an error if an incorrect parameters object is specified", {
    expect_error(tar_spectral_binning(test,
                                      parameters = 'wrong'))
})

test_that("tar_input_spectral_binning() works if `mzML`,`sample_info` and `parameters` arguments are specified directly", {
